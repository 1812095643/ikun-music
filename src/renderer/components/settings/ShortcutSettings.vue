...
async function loadShortcutsFromMain() {
  ...
  const platformValue = window.desktop.sendSync('get-platform');
  if (platformValue === 'darwin' || platformValue === 'win32' || platformValue === 'linux') {
    platform.value = platformValue;
  }

  try {
    const shortcuts = await window.desktop.invoke('shortcuts:get-config');
  ...
  const stored = window.desktop.sendSync('get-store-value', 'shortcuts');
...
async function handleOpen() {
  ...
  if (isDesktopRuntime) {
    ...
    window.desktop.send('disable-shortcuts');
  }
...
function resumeShortcutRuntime() {
  ...
  if (isDesktopRuntime) {
    window.desktop.send('enable-shortcuts');
  }
}
